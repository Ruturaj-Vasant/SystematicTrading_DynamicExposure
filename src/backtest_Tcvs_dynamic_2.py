...
# --- Apply Strategy with Dynamic Exposure ---
def apply_strategy(df):
    df['Return'] = df['SPY_Close'].pct_change()

    def exposure_logic(row):
        if row['Trend_5_20'] == 1 and row['High_Vol'] == 1:
            return min(1.5, 1 + 0.5 * row['Proba'])  # leverage up
        elif row['Trend_5_20'] == 0 and row['High_Vol'] == 1:
            return 0.5 * row['Proba']  # defensive
        else:
            return row['Proba']  # base exposure
...
def plot_equity_curve(cumulative):
    ...
    plt.plot(cumulative.index, 100 * cumulative, label='TCVS Dynamic Strategy')
    plt.title("Cumulative Return of TCVS Dynamic Strategy")
    plt.xlabel("Date")
