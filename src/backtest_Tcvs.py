...
# # --- Apply Trend-Conditioned Volatility Scaling ---
# def apply_tcvs_strategy(df):
...
#             scaling.append(0)
#         elif row['High_Vol']:
#             scaling.append(2 if row['Trend'] == 'Up' else 0.5)
#         else:
#             scaling.append(1)
...
#     plt.title("Cumulative Return of TCVS Strategy")
#     plt.xlabel("Date")
...
def apply_tcvs_strategy(df):
    ...
    df['Scaling_Factor'] = df.apply(dynamic_exposure, axis=1)
    df['Position'] = df['Signal'].shift(1) * df['Scaling_Factor']
...
def evaluate(df):
    ...
    leveraged_days = df['Position'].gt(1).sum()
